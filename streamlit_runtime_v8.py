...
''',
'''    slug = filter_slugs.get(label_text)
    selected_style = ""
    overlay_html = ""
    if slug:
        current = st.query_params.get("dash", "all")
        ...
        overlay_html = (
            f'<a class="kpi-click-overlay" href="?dash={slug}" target="_self" '
            f'aria-label="Filtrar por {escape(label_text)}"></a>'
        )
    ...
    return self.markdown(html, unsafe_allow_html=True)
''',
"KPIs clicáveis sem alterar o visual",
)

...
# Só o primeiro após o editor; há outro no dashboard já substituído, então procure no trecho do data_editor.
editor_pos = text.find('edited_view = st.data_editor(')
if editor_pos < 0:
    raise SystemExit('Editor do Cronograma não encontrado.')
...
wrapper_path = Path('streamlit_app.py')
wrapper = wrapper_path.read_text(encoding='utf-8')
old_parse = 'for col in ["data_separacao", "ultima_alteracao_cronograma"]:'
new_parse = 'for col in ["data_separacao", "ultima_alteracao_cronograma", "ultima_alteracao_equipe"]:'
if old_parse not in wrapper:
    raise SystemExit('Conversão de datas do Supabase não encontrada no wrapper.')
wrapper = wrapper.replace(old_parse, new_parse, 1)
wrapper_path.write_text(wrapper, encoding='utf-8')

print('Últimas alterações de cronograma e equipe aplicadas.')
